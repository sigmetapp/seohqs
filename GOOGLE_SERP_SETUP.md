...
### Шаг 4: Получение Search Engine ID (CX)

⚠️ **Важно:** Google Programmable Search Engine требует указать конкретные сайты для поиска. Для поиска по всему интернету есть два варианта:

#### Вариант А: Использование публичного Search Engine ID (Самый простой способ)

Google предоставляет публичный Search Engine ID, который позволяет искать по всему интернету. Вы можете использовать готовый ID:

**`017576662512468239146:omuauf_lfve`**

Просто используйте этот ID в переменной окружения `GOOGLE_SEARCH_ENGINE_ID` - никаких дополнительных настроек не требуется!

#### Вариант Б: Создание собственного Search Engine (Ограниченный поиск)

Если вы хотите создать свой Search Engine (но он будет искать только по указанным доменам):
...
3. Заполните форму:
   - **Sites to search**: Добавьте домены, по которым хотите искать (каждый с новой строки), например:
     ```
     wikipedia.org
     reddit.com
     stackoverflow.com
...
**⚠️ Важно:** Этот вариант будет искать **только** по указанным доменам, а не по всему интернету!

**Рекомендация:** Используйте Вариант А (публичный ID) для поиска по всему интернету, или рассмотрите альтернативные API (SerpAPI/Zenserp) для более гибкого поиска.
...
⚠️ **Ограничение Google Custom Search API:**
- При использовании собственного Search Engine поиск ограничен указанными доменами
- Для поиска по всему интернету рекомендуется использовать публичный Search Engine ID или альтернативные API (SerpAPI/Zenserp)
