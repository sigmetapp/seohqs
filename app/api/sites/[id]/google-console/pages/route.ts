...
export async function GET(
  ...
  try {
    ...
    // Получаем данные по страницам за последние 30 дней
    const pageData = await searchConsoleService.getPageData(siteUrl, 30, 100);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Ошибка получения данных по страницам',
      },
      { status: 500 }
    );
  }
