...
export async function GET(
  ...
  try {
    ...
    // Получаем данные по странам за последние 30 дней
    const countryData = await searchConsoleService.getCountryData(siteUrl, 30);
    ...
    return NextResponse.json(
      {
        success: false,
        error: error.message || 'Ошибка получения данных по странам',
      },
      { status: 500 }
    );
  }
