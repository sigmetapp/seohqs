 ...
 * Удаляет все сайты пользователя и все связанные данные Google Search Console
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (useSupabase) {
      ...
      // Удаляем сайты
      await db.query('DELETE FROM sites WHERE user_id = $1', [user.id]);
    } else {
      ...
      // Удаляем сайты
      db.prepare('DELETE FROM sites WHERE user_id = ?').run(user.id);
